...
final class AreaZoneIndex {
    ...
    private static Node build(List<Entry> entries, int fromIndex, int toIndex) {
        ...
        if (size <= MAX_LEAF_SIZE) {
            return new Node(bounds, entries.subList(fromIndex, toIndex).toArray(Entry[]::new));
        }

        Axis splitAxis = bounds.longestAxis();
        entries.subList(fromIndex, toIndex).sort(Comparator.comparingLong(entry -> entry.center(splitAxis)));
    ...
    private static final class Entry {
        ...
        private long center(Axis axis) {
            return switch (axis) {
                case X -> (long) minX + maxX;
                case Y -> (long) minY + maxY;
                case Z -> (long) minZ + maxZ;
            };
        }
