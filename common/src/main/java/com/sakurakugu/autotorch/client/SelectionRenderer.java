...
public final class SelectionRenderer {
    ...
    public static void extract(BlockPos fallback) {
        ...
        boolean dynamicDraft = SelectionState.isOverlayEnabled()
                && SelectionState.drafting() && !SelectionState.hasPoints();
        if (renderData != null && renderRevision == revision && !dynamicDraft) {
            return;
        }
    ...
    public static void render(Vec3d camera) {
        ...
        renderZones(Pose.INSTANCE, new VertexConsumer(builder), data, camera);
        tesselator.draw();
    ...
    private static void renderLineGeometry(Vec3d camera, RenderData data) {
        ...
        renderZones(Pose.INSTANCE, null, data, camera);
    ...
    private static void renderZones(Pose pose, VertexConsumer buffer, RenderData data, Vec3d camera) {
        if (data.draft() != null) {
        ...
        if (data.displayMode() == SelectionState.DisplayMode.LINES) {
            if (zone.shape() == AreaShape.SPHERE) {
                ...
                renderBoxLines(pose, buffer,
                        fullBlockBounds(zone), lineColor, width, camera);
            }
    ...
    private static void renderBoxFaces(Pose pose, VertexConsumer buffer, AxisAlignedBB box, int color, Vec3d camera) {
        ...
        quad(pose, buffer, box.maxX, box.minY, box.minZ, box.minX, box.minY, box.minZ,
                box.minX, box.maxY, box.minZ, box.maxX, box.maxY, box.maxZ, color, camera);
    ...
    private static void renderSphereLines(
            Pose pose, VertexConsumer buffer, AreaZone zone, int color, float width, Vec3d camera
    ) {
    ...
    private static float scaledLineWidth(float baseWidth, Vec3d camera, RenderData data) {
    ...
    private static double distanceSquared(BlockPos pos, Vec3d camera) {
    ...
    private static void renderSphereFaces(Pose pose, VertexConsumer buffer, AreaZone zone, int color, Vec3d camera) {
        double cx = zone.first().getX() + 0.5;
        double cy = zone.first().getY() + 0.5;
        double cz = zone.first().getZ() + 0.5;
        double radius = Math.sqrt(zone.radiusSquared()) + 0.5;
    ...
    private static void renderBlockySphereFaces(
            Pose pose, VertexConsumer buffer, AreaZone zone, BlockySphereMesh mesh, int color, Vec3d camera
    ) {
    ...
    private static void renderBlockySphereLines(
            Pose pose, VertexConsumer buffer, AreaZone zone, BlockySphereMesh mesh, int color, float width, Vec3d camera
    ) {
        BlockPos center = zone.first();
    ...
    private static void blockEdge(
            Pose pose, VertexConsumer buffer, BlockPos center, int encodedEdge, int color, float width, Vec3d camera
    ) {
    ...
    private static void blockFaceStrip(
            Pose pose, VertexConsumer buffer, BlockPos center, int encodedFace, int length, int color, Vec3d camera
    ) {
    ...
    private static void sphereQuad(
            ...
            int latitude, int longitude, int color, Vec3d camera
    ) {
    ...
    private static void sphereVertex(
        ...
        applyColor(buffer.vertex(pose.pose(),
                (float) (cx + SPHERE_LONGITUDE_COS[longitude] * horizontal - camera.xCoord),
                (float) (cy + SPHERE_LATITUDE_SIN[latitude] * radius - camera.yCoord),
                (float) (cz + SPHERE_LONGITUDE_SIN[longitude] * horizontal - camera.zCoord)), color).endVertex();
    }

    private static void quad(
            Pose pose, VertexConsumer buffer,
            double x1, double y1, double z1, double x2, double y2, double z2,
            double x3, double y3, double z3, double x4, double y4, double z4, int color, Vec3d camera
    ) {
        // 选区面使用独立四边形且不写深度，保证水面等透明内容仍能正常渲染。
        // 先转换为相机相对坐标，再转为 float，避免远离世界原点时的精度损失。
        applyColor(buffer.vertex(pose.pose(), (float) (x1 - camera.xCoord), (float) (y1 - camera.yCoord), (float) (z1 - camera.zCoord)), color).endVertex();
        applyColor(buffer.vertex(pose.pose(), (float) (x2 - camera.xCoord), (float) (y2 - camera.yCoord), (float) (z2 - camera.zCoord)), color).endVertex();
        applyColor(buffer.vertex(pose.pose(), (float) (x3 - camera.xCoord), (float) (y3 - camera.yCoord), (float) (z3 - camera.zCoord)), color).endVertex();
        applyColor(buffer.vertex(pose.pose(), (float) (x4 - camera.xCoord), (float) (y4 - camera.yCoord), (float) (z4 - camera.zCoord)), color).endVertex();
    }

    private static void line(
            Pose pose, VertexConsumer buffer,
            double x1, double y1, double z1, double x2, double y2, double z2,
            int color, float width, Vec3d camera
    ) {
        // 先转换为相机相对坐标，再转为 float，避免远离世界原点时的精度损失。
