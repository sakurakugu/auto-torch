...
public final class LightingScreen extends Screen {
    ...
    private Component error = new TextComponent("");
    ...
    protected void init() {
        tooltips.clear();
        BlockPos playerPos = minecraft.player == null ? BlockPos.ZERO : minecraft.player.getCommandSenderBlockPosition();
        int left = panelLeft();
        ...
        sphereDisplayButton = addButton(button(sphereDisplayMessage(), button -> {
            ...
            sphereDisplayButton.setMessage(sphereDisplayMessage().getString());
        }).bounds(left + 216, 88, 94, 20).build());
        displayButton = addButton(button(displayMessage(), button -> {
            cycleSelectionDisplay();
            displayButton.setMessage(displayMessage().getString());
        ...
        lightOverlayRenderThroughButton = addButton(button(lightOverlayRenderThroughMessage(), button -> {
            ...
            lightOverlayRenderThroughButton.setMessage(lightOverlayRenderThroughMessage().getString());
        ...
        swampSlimeDetectionButton = addButton(button(swampSlimeDetectionMessage(), button -> { }).bounds(left, 306, 116, 20)
        ...
        drownedDetectionButton = addButton(button(drownedDetectionMessage(), button -> {
            ...
            drownedDetectionButton.setMessage(drownedDetectionMessage().getString());
                ...
                .build());
        addButton(button(new TranslatableComponent("screen.autotorch.more_settings"), button -> {
            saveSelection();
            saveTaskDefaults();
        ...
        addButton(new ResetButton(resetButtonX(nearbyResetMessage), 334,
                font.width(nearbyResetMessage.getString()), 16, nearbyResetMessage, panelLeft(), panelLeft() + 310,
    ...
    private void onDimensionChanged() {
        ...
        try {
            ...
            error = new TextComponent("");
            if (dimensionClamped) {
                rangeMessage = new TranslatableComponent(SelectionState.shape() == AreaShape.SPHERE
    ...
    private void updateSelectionRangeMessage(BlockPos firstPos, BlockPos secondPos) {
        if (SelectionState.shape() == AreaShape.SPHERE) {
            ...
            rangeMessage = selectionInRange ? new TextComponent("")
                    : new TranslatableComponent("screen.autotorch.sphere_radius_too_large",
                    ServerConfigState.maxSphereRadius());
        } else {
            ...
            rangeMessage = selectionInRange ? new TextComponent("")
                    : new TranslatableComponent("screen.autotorch.box_axis_too_large",
                    ServerConfigState.maxBoxAxisLength());
        }
        updateZoneButtonAvailability();
    }
    ...
    private void swapPoints() {
        try {
            ...
            error = new TranslatableComponent("screen.autotorch.invalid_value");
        }
    }
        ...
        try {
            ...
            if (SelectionState.shape() == AreaShape.BOX) {
                ...
                if (radius * 2L + 1L > ServerConfigState.maxBoxAxisLength()) {
                    error = new TranslatableComponent(
    ...
    private void addExclusion() {
        try {
            ...
            if (!SelectionState.addExclusion(zone)) {
                error = new TranslatableComponent("screen.autotorch.too_many_exclusions");
            } else {
                exclusionButton.setMessage(exclusionMessage().getString());
                error = new TextComponent("");
            }
    ...
    private void startTask() {
        try {
            ...
            if (selection == null) {
                error = new TranslatableComponent("screen.autotorch.no_lighting_zone");
                return;
            }
            if (SelectionState.drafting()
                ...
                error = new TranslatableComponent("screen.autotorch.confirm_draft");
                return;
            }
    ...
    private Component shapeMessage() {
        return new TranslatableComponent(SelectionState.shape() == AreaShape.SPHERE
    ...
    private Component exclusionMessage() {
        return new TranslatableComponent(SelectionState.isEditingExclusion()
    ...
    private Component displayMessage() {
        if (!SelectionState.isOverlayEnabled()) {
            return new TranslatableComponent("screen.autotorch.display_off");
        }
        return new TranslatableComponent(SelectionState.displayMode() == SelectionState.DisplayMode.FACES
                ? "screen.autotorch.display_faces" : "screen.autotorch.display_lines");
    }

    private Component sphereDisplayMessage() {
        return new TranslatableComponent(SelectionState.sphereDisplayMode() == SelectionState.SphereDisplayMode.BLOCKY
    ...
    private Component consumeMessage() {
        if (!canChooseConsumeTorches()) {
            return new TranslatableComponent(ServerConfigState.survivalConsumesTorches()
                    ? "screen.autotorch.consume_on" : "screen.autotorch.consume_off");
        }
        return new TranslatableComponent(consumeTorches ? "screen.autotorch.consume_on" : "screen.autotorch.consume_off");
    }

    private Component undergroundMessage() {
        return new TranslatableComponent(undergroundOnly ? "screen.autotorch.underground_on" : "screen.autotorch.underground_off");
    }

    private Component lightOverlayMessage() {
        return new TranslatableComponent(LightOverlayState.isEnabled()
                ? "screen.autotorch.light_overlay_on" : "screen.autotorch.light_overlay_off");
    }

    private Component lightOverlayModeMessage() {
        String key;
        switch (LightOverlayState.displayMode()) {
            case NUMBERS:
                key = "screen.autotorch.light_overlay_mode_numbers";
                break;
            case BOXED_NUMBERS:
                key = "screen.autotorch.light_overlay_mode_boxed_numbers";
                break;
            default:
                key = "screen.autotorch.light_overlay_mode_crosses";
                break;
        }
    ...
    private Component nearbyAutoTorchMessage() {
        return new TranslatableComponent(ClientConfig.isNearbyAutoTorchEnabled()
    ...
    private Component nearbyAutoTorchSkyLightMessage() {
        return new TranslatableComponent(ClientConfig.includesSkyLight()
    ...
    private Component woodenAxeSelectionMessage() {
        return new TranslatableComponent(ClientConfig.isWoodenAxeSelectionEnabled()
    ...
    public void render(int mouseX, int mouseY, float partialTick) {
        ...
        if (sphere) {
            drawString(font, new TranslatableComponent("screen.autotorch.radius_label").getString(), left + 2, 94 - offset, 0xFF70A0FF);
        } else {
            drawString(font, new TranslatableComponent("screen.autotorch.length_label").getString(), left + 2, 94 - offset, 0xFF70A0FF);
            drawString(font, new TranslatableComponent("screen.autotorch.width_label").getString(), left + 104, 94 - offset, 0xFF70A0FF);
            drawString(font, new TranslatableComponent("screen.autotorch.height_label").getString(), left + 206, 94 - offset, 0xFF70A0FF);
        }
        drawString(font, new TranslatableComponent("screen.autotorch.max_torches").getString(), left, 166 - offset, 0xFFFFFFFF);
        ...
        if (!error.getString().isEmpty()) {
            ...
            drawCenteredString(font, new TranslatableComponent("screen.autotorch.light_threshold_unreachable",
                    ClientConfig.defaultTaskLightThreshold()).getString(), width / 2, informationY, 0xFFFF6060);
        } else if (ServerConfigState.minSpacing() > AutoTorchRules.maxSafeSecondPassSpacing(
                ClientConfig.defaultTaskLightThreshold())) {
            drawCenteredString(font, new TranslatableComponent("screen.autotorch.second_pass_spacing_limited",
                    effectiveSecondPassSpacing(), ClientConfig.defaultTaskLightThreshold()).getString(),
                    width / 2, informationY, 0xFFFFC060);
        } else {
            drawString(font, new TranslatableComponent("screen.autotorch.zone_summary",
        ...
        drawCenteredString(font, new TranslatableComponent("screen.autotorch.light_overlay_title").getString(),
                width / 2, 246 - offset, 0xFFFFFFFF);
        fill(left, 334 - offset, left + 310, 335 - offset, 0xFF606060);
        drawCenteredString(font, new TranslatableComponent("screen.autotorch.nearby_auto_torch_title").getString(),
        ...
        if (maxScrollOffset() > 0) {
            ...
            fill(x, VIEWPORT_MARGIN, x + SCROLLBAR_WIDTH, height - VIEWPORT_MARGIN, 0x80000000);
    ...
    private static final class LightRangeSlider extends ScrollingSliderButton {
        ...
        protected void updateMessage() {
            setMessage(new TranslatableComponent("screen.autotorch.light_overlay_range_value", range()).getString());
        }
    ...
    private final class MinSpacingSlider extends ScrollingSliderButton {
        ...
        protected void updateMessage() {
            int spacing = spacing();
            setMessage(new TranslatableComponent("screen.autotorch.min_spacing", spacing).getString());
            // 1.15.2 没有 Tooltip API，改用本界面的工具提示表。
    ...
    private static final class AreaLightThresholdSlider extends ScrollingSliderButton {
        ...
        @Override
        protected void updateMessage() {
            setMessage(new TranslatableComponent("screen.autotorch.area_light_threshold", threshold()).getString());
        }
