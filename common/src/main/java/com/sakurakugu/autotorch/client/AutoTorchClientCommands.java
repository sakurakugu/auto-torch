...
public final class AutoTorchClientCommands {
    ...
    private static IChatComponent overlayEnabledText() {
        return ClientConfig.isLightOverlayRenderThrough()
                ? new ChatComponentTranslation("command.autotorch.status.overlay_see_through",
                        state(LightOverlayState.isEnabled()))
                : state(LightOverlayState.isEnabled());
    }

    /** 光照显示样式文本，数字样式且跟随视角时附加对应的标记。 */
    private static IChatComponent overlayModeText() {
        ...
        return rotatesNumbers
                ? new ChatComponentTranslation("command.autotorch.status.overlay_rotate_numbers",
                        new ChatComponentTranslation(displayModeKey(mode)))
                : new ChatComponentTranslation(displayModeKey(mode));
    }
    ...
    private static int setOverlayMode(LightOverlayState.DisplayMode mode) {
        ...
        return feedback("command.autotorch.overlay_mode", new ChatComponentTranslation(displayModeKey(mode)));
    }

    private static <S> BlockPos position(CommandContext<S> context, String name) {
        Minecraft minecraft = Minecraft.getMinecraft();
        Vec3 origin = minecraft.thePlayer == null
                ? new Vec3(0.0D, 0.0D, 0.0D) : minecraft.thePlayer.getPositionVector();
        String[] values = {
        ...
        if (values[0].startsWith("^") && minecraft.thePlayer != null) {
            ...
            Vec3 leftVector = new Vec3(-cross.xCoord, -cross.yCoord, -cross.zCoord);
            Vec3 eye = new Vec3(minecraft.thePlayer.posX,
        ...
        if (minecraft.thePlayer != null) {
            ...
            feedbackColored("command.autotorch.status.task", STATUS_DETAILS_COLOR,
                    ...
                    new ChatComponentTranslation(sphere
                            ? "command.autotorch.shape_sphere" : "command.autotorch.shape_box"),
                    state(LightOverlayState.isEnabled()),
                    new ChatComponentTranslation(displayModeKey(LightOverlayState.displayMode())));
        }
        feedbackColored("------------------------------------------------", EnumChatFormatting.WHITE);
        return 1;
    }
