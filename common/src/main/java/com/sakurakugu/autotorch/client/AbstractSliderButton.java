...
abstract class AbstractSliderButton extends Button {
    ...
    protected void mouseDragged(Minecraft minecraft, int mouseX, int mouseY) {
        // 1.7.10 绘制按钮时也会调用此方法，只有按住滑块后才更新数值。
        if (dragging) {
            updateFromMouse(mouseX);
        }
    }
    ...
    protected void renderButton(int mouseX, int mouseY, float partialTicks) {
        ...
        drawTexturedModalRect(handleLeft, yPosition, 0, textureY, 4, height);
        drawTexturedModalRect(handleLeft + 4, yPosition, 196, textureY, 4, height);
