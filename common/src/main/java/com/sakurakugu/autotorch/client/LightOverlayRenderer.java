...
public final class LightOverlayRenderer {
    ...
    public static void render(Vec3 camera, PoseStack poseStack, MultiBufferSource buffers) {
        ...
        if (data.displayMode() != LightOverlayState.DisplayMode.CROSSES) {
            ...
            RenderType numberRenderType = ClientConfig.isLightOverlayRenderThrough()
                    ? AutoTorchRenderTypes.numbers(numberTexture, true) : RenderType.text(numberTexture);
    ...
    /** 结束数字纹理批次，确保顶点在当前世界渲染矩阵仍有效时提交。 */
    public static void endBatches(MultiBufferSource.BufferSource buffers) {
        RenderData data = renderData;
        if (data == null || data.displayMode() == LightOverlayState.DisplayMode.CROSSES) {
            return;
        }
    ...
    private static void submitNumberQuad(
        ...
        // 1.20.6 的 POSITION_COLOR_TEX_LIGHTMAP 要求按颜色、UV、光照 UV 的顺序提交属性。
        buffer.vertex(pose, topLeftX, y, topLeftZ).color(color).uv(u, v)
                .uv2(FULL_BRIGHT_LIGHT, FULL_BRIGHT_LIGHT).endVertex();
        buffer.vertex(pose, bottomLeftX, y, bottomLeftZ).color(color)
                .uv(u, v + NUMBER_TEXTURE_CELL_SIZE)
                .uv2(FULL_BRIGHT_LIGHT, FULL_BRIGHT_LIGHT).endVertex();
        buffer.vertex(pose, bottomRightX, y, bottomRightZ).color(color)
                .uv(u + NUMBER_TEXTURE_CELL_SIZE, v + NUMBER_TEXTURE_CELL_SIZE)
                .uv2(FULL_BRIGHT_LIGHT, FULL_BRIGHT_LIGHT).endVertex();
        buffer.vertex(pose, topRightX, y, topRightZ).color(color)
