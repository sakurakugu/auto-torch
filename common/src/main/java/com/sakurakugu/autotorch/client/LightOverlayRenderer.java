...
public final class LightOverlayRenderer {
            ...
            if (drowned != null && drowned.renderableCount() > 0) {
                if (data.displayMode() == LightOverlayState.DisplayMode.CROSSES) {
                    ...
                    renderGeometry(drowned, camera, poseStack, buffers.getBuffer(type),
                            (pose, buffer) -> submitNumbers(pose, buffer, drowned, camera, numberDirection));
                    if (data.displayMode() == LightOverlayState.DisplayMode.BOXED_NUMBERS) {
                        renderLines(camera, poseStack, buffers, drowned, true);
                    }
                }
    ...
    private static void submitNumbers(
        ...
        for (ColumnRenderData column : data.columns()) {
            for (NumberQuad quad : column.numberQuads()) {
                ...
                switch (direction) {
                    case SOUTH:
                        submitNumberQuad(pose, buffer, quad.color(), x + size, y, z + size,
                                x + size, z, x, z, x, z + size, u, v);
                        break;
                    case EAST:
                        submitNumberQuad(pose, buffer, quad.color(), x + size, y, z,
                                x, z, x, z + size, x + size, z + size, u, v);
                        break;
                    case WEST:
                        submitNumberQuad(pose, buffer, quad.color(), x, y, z + size,
                                x + size, z + size, x + size, z, x, z, u, v);
                        break;
                    default:
                        submitNumberQuad(pose, buffer, quad.color(), x, y, z,
    ...
    private static void submitNumberQuad(
        ...
        // 1.15.2 的 POSITION_COLOR_TEX_LIGHTMAP 格式要求按颜色、UV、光照 UV 的顺序提交属性。
        buffer.vertex(pose.pose(), topLeftX, y, topLeftZ)
                .color((color >> 16) & 0xFF, (color >> 8) & 0xFF, color & 0xFF, color >>> 24)
                .uv(u, v).uv2(FULL_BRIGHT_LIGHT, FULL_BRIGHT_LIGHT).endVertex();
        buffer.vertex(pose.pose(), bottomLeftX, y, bottomLeftZ)
                .color((color >> 16) & 0xFF, (color >> 8) & 0xFF, color & 0xFF, color >>> 24)
                .uv(u, v + NUMBER_TEXTURE_CELL_SIZE)
                .uv2(FULL_BRIGHT_LIGHT, FULL_BRIGHT_LIGHT).endVertex();
        buffer.vertex(pose.pose(), bottomRightX, y, bottomRightZ)
                .color((color >> 16) & 0xFF, (color >> 8) & 0xFF, color & 0xFF, color >>> 24)
                .uv(u + NUMBER_TEXTURE_CELL_SIZE, v + NUMBER_TEXTURE_CELL_SIZE)
                .uv2(FULL_BRIGHT_LIGHT, FULL_BRIGHT_LIGHT).endVertex();
        buffer.vertex(pose.pose(), topRightX, y, topRightZ)
                .color((color >> 16) & 0xFF, (color >> 8) & 0xFF, color & 0xFF, color >>> 24)
