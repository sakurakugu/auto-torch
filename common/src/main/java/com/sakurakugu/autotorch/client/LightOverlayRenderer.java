...
public final class LightOverlayRenderer {
    ...
    public static void submit(Vec3 camera, PoseStack poseStack, SubmitNodeCollector collector) {
        ...
        renderGeometry(camera, poseStack,
                (stack, geometry) -> collector.submitCustomGeometry(stack, renderType, geometry::render), renderer);
    }
    ...
    public static void render(Vec3 camera, PoseStack poseStack, MultiBufferSource.BufferSource buffers) {
        ...
        GeometryRenderer renderer = data.displayMode() == LightOverlayState.DisplayMode.NUMBERS
                ...
                : (pose, buffer) -> submitLines(pose, buffer, data, camera);
        renderGeometry(camera, poseStack,
                (stack, geometry) -> geometry.render(stack.last(), buffers.getBuffer(renderType)), renderer);
    }
    ...
    private static void renderGeometry(
        ...
        sink.submit(poseStack, renderer);
        poseStack.popPose();
    }
