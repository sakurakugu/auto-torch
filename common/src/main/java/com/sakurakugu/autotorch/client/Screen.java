...
/** 集中适配 1.12.2 GuiScreen 与界面业务使用的控件生命周期。 */
abstract class Screen extends GuiScreen {
    ...
    @Override public final void initGui() { minecraft = mc; font = fontRenderer; buttonList.clear(); children.clear(); init(); }
    ...
    protected void renderTooltip(String text, int mouseX, int mouseY) {
        // 1.12.2 的字符串重载不会拆分换行，先按行拆分再交给多行提示渲染。
    ...
    public boolean mouseReleased(double mouseX, double mouseY, int button) { super.mouseReleased((int) mouseX, (int) mouseY, button); return true; }
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double dx, double dy) {
        if (selectedButton instanceof Button) ((Button) selectedButton).drag((int) mouseX, (int) mouseY);
