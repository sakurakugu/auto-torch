...
/** 将服务端配置命令接入 Forge 1.7.10 的 ICommand API。 */
final class LegacyAutoTorchServerCommand extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] arguments) throws CommandException {
        ...
        if (arguments.length == 0 || "help".equals(arguments[0])) {
            sender.addChatMessage(new ChatComponentTranslation("command.autotorch.server.help"));
            return;
        }
        ...
        if ("defaults".equals(action)) {
            ...
            sync(server);
            func_152373_a(sender, this, "command.autotorch.server.defaults");
            return;
        }
        ...
        Object value = parseValue(sender, definition, arguments[3]);
        ServerConfig.set(key, value);
        sync(server);
        func_152373_a(sender, this, "command.autotorch.server.value", key, ServerConfig.get(key));
    }

    private static Object parseValue(ICommandSender sender, Value definition, String raw) throws CommandException {
        ...
        if (definition instanceof IntValue) {
            ...
            return parseIntBounded(sender, raw, intValue.minValue(), intValue.maxValue());
        }
