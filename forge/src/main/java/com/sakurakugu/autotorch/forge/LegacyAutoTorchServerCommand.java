...
/** 将共享服务端配置命令接入 Forge 1.12.2 的 ICommand API。 */
final class LegacyAutoTorchServerCommand extends CommandBase {
    @Override public String getName() { return "autotorch"; }
    @Override public String getUsage(ICommandSender sender) { return "command.autotorch.server.help"; }
    @Override public int getRequiredPermissionLevel() { return 2; }

    @Override
    public void execute(MinecraftServer server, ICommandSender sender, String[] arguments) throws CommandException {
        if (arguments.length == 0 || "help".equals(arguments[0])) {
            sender.sendMessage(new TextComponentTranslation("command.autotorch.server.help"));
            return;
        }
