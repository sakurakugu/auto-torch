...
适用于 Minecraft 1.18.2 / Forge 和 Fabric 的自动插火把和光照显示模组。
...
该版本因为 Forge 发布 JAR 没有执行 SRG 重映射，导致开发环境方法名 `Minecraft.getInstance()` 被直接发布，生产环境只能识别 `m_91087_()`。
目前我修复了这个问题，因此导致生产端打得开，但是开发端打不开。
...
- 以玩家为中心增量扫描周围区域，显示范围水平可设为 1~64 格，纵向可选上下各64格（最大 64）；玩家移动后会自动刷新，扫描过程经过分帧处理。
- 支持 `X` 标记、数字和方框数字三种显示样式。数字为方块光照等级，颜色同时表示对应位置的刷新风险。
- 支持透视渲染，数字跟随视角旋转。
...
- 扫描分两轮执行：第一轮使用设定的最小间距，第二轮使用较小间距补齐仍未照亮的位置。第二轮会根据亮度要求自动收紧间距，避免高亮度目标因间距规则而跳过暗点；服务器设置的最小间距过大时，面板和任务开始消息会提示目标可能无法满足。间距是本次任务所放火把方块坐标之间的三维直线距离，不会计算区间内原有火把；例如间距为 `8` 时，同一直线上的两个火把之间有 7 格。扫描量和放置量均按 tick 限流，多个玩家的任务会轮流获得预算。火把最高只能提供 14 级方块光，因此只有亮度上限设为 15 时无法完全满足。
...
horizontalRange = 16
# 相对玩家向下扫描的格数，范围 0~64；与 upRange 之和不超过 64。
downRange = 16
# 相对玩家向上扫描的格数，范围 0~64；与 downRange 之和不超过 64。
upRange = 4
...
- 基岩版移植后发现它对自定义绘制和UI配置等都不如Java友好，于是取消了基岩版移植计划，如果有兴趣可以自行移植。
  - 至于基岩版的 LeviLamina ，评估后所有内容都是可以支持的，但是因为基岩版没有生态，因此不打算移植。
- Java版本 1.7.10 以下因为需要大幅修改，且Forge官方也没找到下载入口，因此不打算支持。
