...
适用于 Minecraft 1.21.11~26.2 / NeoForge、Forge 和 Fabric 的自动插火把和光照显示模组。
...
- 可设置任务的方块光上限（0~15），任务处理方块光小于或等于该值、脚部和头部为空气、脚下可站立的位置。默认值为 `0`，即只处理方块光为 `0` 的位置。“仅地下”开启时还会跳过有天空光的位置。
- 火把位置优先选择暗点脚下，无法放置时会在附近随机寻找有效位置；只处理已加载区块，不会强制加载区块。
- 扫描分两轮执行：第一轮使用设定的最小间距，第二轮使用较小间距补齐仍未照亮的位置。间距是本次任务所放火把方块坐标之间的三维直线距离，不会计算区间内原有火把；例如间距为 `8` 时，同一直线上的两个火把之间有 7 格。扫描量和放置量均按 tick 限流，多个玩家的任务会轮流获得预算。
...
- 默认情况下长方体每条边最多 321 格，球形半径最多 160 格（直径 321 格）；服主可以在允许范围内调低配置。
...
- 网易版不打算开发，不仅不支持中文路径，而且还污染环境，乱扔文件夹，卸载后还有一堆残留。因此没有网易版开发计划。
- 基岩版移植后发现它对自定义绘制和UI配置等都不如Java友好，于是取消了基岩版移植计划，如果有兴趣可以自行移植。
