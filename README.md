...
- 默认按 `F7` 开关纯客户端光照显示，也可在 `G` 面板中切换；支持 X 和方块光等级数字两种样式，并可通过滑动条设置 `1～64` 格的水平显示半径（默认 16）。X 只标记危险位置，数字显示所有符合条件的位置；红色表示昼夜都有风险，黄色表示夜间风险，绿色表示安全。
- 使用 A、B 两点选择球形或长方体区间；球形以 A 为圆心、A 到 B 的距离为半径。
- 手持木斧左键方块设置 A，右键方块设置 B，操作方式与 WorldEdit 类似。
- A/B 草稿显示为蓝色，确认后的扫描范围显示为绿色，所有排除区显示为红色。
- 范围默认以半透明面显示，也可在面板中切换为线框或独立关闭选区显示；此开关不影响光照显示。
