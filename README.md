...
- 默认按 `F7` 开关纯客户端光照显示，也可在 `G` 面板中切换；支持 X 和方块光等级数字两种样式，并可通过滑动条设置 `1～64` 格的水平显示半径（默认 16）。X 只标记危险位置，数字显示所有符合条件的位置；红色表示昼夜都有风险，黄色表示夜间风险，绿色表示安全。面板中另有默认关闭的沼泽史莱姆和溺尸特判开关，紫色表示沼泽史莱姆条件风险，青色表示溺尸风险。
...
- 选区显示默认关闭，面板中的显示按钮可在关、面和线之间循环；此设置不影响光照显示。
...
- 生存模式固定不消耗背包火把；创造模式可以选择是否消耗，默认关闭。
...
- `<世界目录>/serverconfig/autotorch-server.toml`：保存选区尺寸、火把数量、排除区、并发任务和每 tick 工作预算等服务端限制。

服务端始终以服务端配置和服务端校验为准。生存玩家即使发送修改过的数据包，也会被强制设为不消耗背包火把。
...
- 无限模式仍会受选区内有效位置限制；创造模式启用消耗时还会受背包火把数量限制。
